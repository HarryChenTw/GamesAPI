...
def insert_into_mysql_table_for_uba(table_name, games_table, truncat_table = False):
    ...
    insert_command = f"INSERT INTO {table_name}(league, game_type, datetime, weekday, team_1, team_2) VALUES(%s, %s, %s, %s, %s, %s)"
    ...
    with pymysql.connect(**db_settings) as conn :
        with conn.cursor() as cursor:
            ...
            for i in range(len(games_table)):
                cursor.execute(
                        insert_command, (games_table.iloc[i]['league'], games_table.iloc[i]['game_type'], games_table.iloc[i]['datetime'], games_table.iloc[i]['weekday'], games_table.iloc[i]['team_1'], games_table.iloc[i]['team_2'])
                )
            conn.commit()
